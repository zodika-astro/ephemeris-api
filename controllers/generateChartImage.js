...
async function generateNatalChartImage(ephemerisData) {
  ...
  houseCusps.forEach((cusp, index) => {
    ...
    const next = houseCusps[(index + 1) % 12].degree;
    let mid = (cusp.degree + (next > cusp.degree ? next : next + 360)) / 2;
    if (mid >= 360) mid -= 360;
    const r = innerRadius + (planetOrbitRadius - innerRadius) * 0.5;
    const x = centerX + r * Math.cos(degToRad(mid));
    const y = centerY + r * Math.sin(degToRad(mid));
