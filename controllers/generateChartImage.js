...
const fontPaths = {
  ...
  symbola: path.join(__dirname, '../fonts/symbola.ttf')
};

// Verifica se os arquivos existem
Object.entries(fontPaths).forEach(([name, fontPath]) => {
  if (!fs.existsSync(fontPath)) {
    console.error(`Fonte ${name} não encontrada em: ${fontPath}`);
  }
});
...
try {
  ...
  registerFont(fontPaths.symbola, { family: 'Symbola' });
} catch (e) {
  console.warn('Erro ao registrar fontes:', e.message);
}
